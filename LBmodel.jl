...
@variable(model,Gendata[i,10]<=Pg[i in 1:Num_gen]<=Gendata[i,9], start=Gendata[i,9])
@variable(model,Gendata[i,5]<=Qg[i in 1:Num_gen]<=Gendata[i,4], start=Gendata[i,4])
@variable(model,XL[1:Num_Cline],Bin)
@variable(model,0<=Qc[1:Num_bus]<=0)
@variable(model,0<=Ql[1:Num_bus]<=0)
if G==1
    ...
    @variable(model,XG[1:Num_Cgen],Bin)
end
...
if G==1
    ...
    @constraint(model,con_budgetconstraint_gen,sum(Gendata_candidate[i,6]*XG[i] for i in 1:Num_Cgen)<=Maxinvest_cost_gen)
    ...
    sum(Loaddata[j,3] for j in 1:Num_load if Loaddata[j,1]==i)-Bsh[i]*Ui[i]-Qc[i]+Ql[i]==0)
...
@constraint(model,con_CandidateActivelineflowmaxSend[i=1:Num_Cline],-PLc_S[i]+GC_ff[i]*Ui[Linedata_candidate[i,1]]+GC_ft[i]*RC[i]+BC_ft[i]*LC[i]-
((abs(GC_ff[i])+abs(GC_ft[i])+abs(BC_ft[i]))*1.1^2)*(1-XL[i])<=0)
@constraint(model,con_CandidateActivelineflowminSend[i=1:Num_Cline],-PLc_S[i]+GC_ff[i]*Ui[Linedata_candidate[i,1]]+GC_ft[i]*RC[i]+BC_ft[i]*LC[i]+
((abs(GC_ff[i])+abs(GC_ft[i])+abs(BC_ft[i]))*1.1^2)*(1-XL[i])>=0)
...
@constraint(model,con_CandidateReactivelineflowmaxSend[i=1:Num_Cline],-QLc_S[i]-BC_ff[i]*Ui[Linedata_candidate[i,1]]-BC_ft[i]*RC[i]+GC_ft[i]*LC[i]-
((abs(BC_ff[i])+abs(BC_ft[i])+abs(GC_ft[i]))*1.1^2)*(1-XL[i])<=0)
@constraint(model,con_CandidateReactivelineflowminSend[i=1:Num_Cline],-QLc_S[i]-BC_ff[i]*Ui[Linedata_candidate[i,1]]-BC_ft[i]*RC[i]+GC_ft[i]*LC[i]+
((abs(BC_ff[i])+abs(BC_ft[i])+abs(GC_ft[i]))*1.1^2)*(1-XL[i])>=0)
...
@constraint(model,con_CandidateActivelineflowmaxRec[i=1:Num_Cline],-PLc_R[i]+GC_tt[i]*Ui[Linedata_candidate[i,2]]+GC_tf[i]*RC[i]-BC_tf[i]*LC[i]-
((abs(GC_tt[i])+abs(GC_tf[i])+abs(BC_tf[i]))*1.1^2)*(1-XL[i])<=0)
@constraint(model,con_CandidateActivelineflowminRec[i=1:Num_Cline],-PLc_R[i]+GC_tt[i]*Ui[Linedata_candidate[i,2]]+GC_tf[i]*RC[i]-BC_tf[i]*LC[i]+
((abs(GC_tt[i])+abs(GC_tf[i])+abs(BC_tf[i]))*1.1^2)*(1-XL[i])>=0)
...
@constraint(model,con_CandidateReactivelineflowmaxRec[i=1:Num_Cline],-QLc_R[i]-BC_tt[i]*Ui[Linedata_candidate[i,2]]-BC_tf[i]*RC[i]-GC_tf[i]*LC[i]-
((abs(BC_tt[i])+abs(BC_tf[i])+abs(GC_tf[i]))*1.1^2)*(1-XL[i])<=0)
@constraint(model,con_CandidateReactivelineflowminRec[i=1:Num_Cline],-QLc_R[i]-BC_tt[i]*Ui[Linedata_candidate[i,2]]-BC_tf[i]*RC[i]-GC_tf[i]*LC[i]+
((abs(BC_tt[i])+abs(BC_tf[i])+abs(GC_tf[i]))*1.1^2)*(1-XL[i])>=0)
...
@constraint(model,con_conic2[i=1:Num_Cline],RC[i]^2+LC[i]^2<=Ui[Linedata_candidate[i,1]]*Ui[Linedata_candidate[i,2]])
...
if RT>0
   ...
   for i=1:size(C,1)
       ...
       for j=1:size(cyc,1)-1
        ...
        if Flag_cycle[i] !=0        
           #@constraint(model,sum(sign(edge[h])*Theta[edge[h]*sign(edge[h])] for h=1:size(cyc,1) )==0)
           @constraint(model,sum(sign(edge[h])*Theta[Int(edge[h]*sign(edge[h]))] for h=1:size(cyc,1) )==0)  
           for h=1:size(cyc,1) 
               #Flag_conic[edge[h]*sign(edge[h])]=1
               Flag_conic[Int(edge[h]*sign(edge[h]))]=1
           end
        end
   end
...
if RT>1
   ...
   @constraint(model,con_ThetaCL[i=1:Num_Cline],ThetaCL[i]-(theta[Linedata_candidate[i,1]]-theta[Linedata_candidate[i,2]])==0)
   ...
   (lR_CL[i,1]*V_min[Linedata_candidate[i,1]]*V_min[Linedata_candidate[i,2]]*CR_min_CL[i]+
   lR_CL[i,2]*V_min[Linedata_candidate[i,1]]*V_min[Linedata_candidate[i,2]]*CR_max_CL[i]+
   lR_CL[i,3]*V_min[Linedata_candidate[i,1]]*V_max[Linedata_candidate[i,2]]*CR_min_CL[i]+
   lR_CL[i,4]*V_min[Linedata_candidate[i,1]]*V_max[Linedata_candidate[i,2]]*CR_max_CL[i]+
   lR_CL[i,5]*V_max[Linedata_candidate[i,1]]*V_min[Linedata_candidate[i,2]]*CR_min_CL[i]+
   lR_CL[i,6]*V_max[Linedata_candidate[i,1]]*V_min[Linedata_candidate[i,2]]*CR_max_CL[i]+
   lR_CL[i,7]*V_max[Linedata_candidate[i,1]]*V_max[Linedata_candidate[i,2]]*CR_min_CL[i]+
   lR_CL[i,8]*V_max[Linedata_candidate[i,1]]*V_max[Linedata_candidate[i,2]]*CR_max_CL[i])-1000*(1-XL[i])<=0)
   ...
   @constraint(model,[i=1:Num_Cline],Vi[Linedata_candidate[i,1]]-(lR_CL[i,1]*V_min[Linedata_candidate[i,1]]+lR_CL[i,2]*V_min[Linedata_candidate[i,1]]+lR_CL[i,3]*V_min[Linedata_candidate[i,1]]+lR_CL[i,4]*V_min[Linedata_candidate[i,1]]+lR_CL[i,5]*V_max[Linedata_candidate[i,1]]+lR_CL[i,6]*V_max[Linedata_candidate[i,1]]+lR_CL[i,7]*V_max[Linedata_candidate[i,1]]+lR_CL[i,8]*V_max[Linedata_candidate[i,1]])==0)
   @constraint(model,[i=1:Num_Cline],Vi[Linedata_candidate[i,2]]-(lR_CL[i,1]*V_min[Linedata_candidate[i,2]]+lR_CL[i,2]*V_min[Linedata_candidate[i,2]]+lR_CL[i,3]*V_max[Linedata_candidate[i,2]]+lR_CL[i,4]*V_max[Linedata_candidate[i,2]]+lR_CL[i,5]*V_min[Linedata_candidate[i,2]]+lR_CL[i,6]*V_min[Linedata_candidate[i,2]]+lR_CL[i,7]*V_max[Linedata_candidate[i,2]]+lR_CL[i,8]*V_max[Linedata_candidate[i,2]])==0)
   ...
   (lL_CL[i,1]*V_min[Linedata_candidate[i,1]]*V_min[Linedata_candidate[i,2]]*SR_min_CL[i]+
   lL_CL[i,2]*V_min[Linedata_candidate[i,1]]*V_min[Linedata_candidate[i,2]]*SR_max_CL[i]+
   lL_CL[i,3]*V_min[Linedata_candidate[i,1]]*V_max[Linedata_candidate[i,2]]*SR_min_CL[i]+
   lL_CL[i,4]*V_min[Linedata_candidate[i,1]]*V_max[Linedata_candidate[i,2]]*SR_max_CL[i]+
   lL_CL[i,5]*V_max[Linedata_candidate[i,1]]*V_min[Linedata_candidate[i,2]]*SR_min_CL[i]+
   lL_CL[i,6]*V_max[Linedata_candidate[i,1]]*V_min[Linedata_candidate[i,2]]*SR_max_CL[i]+
   lL_CL[i,7]*V_max[Linedata_candidate[i,1]]*V_max[Linedata_candidate[i,2]]*SR_min_CL[i]+
   lL_CL[i,8]*V_max[Linedata_candidate[i,1]]*V_max[Linedata_candidate[i,2]]*SR_max_CL[i])-1000*(1-XL[i])<=0)
   ...
   @constraint(model,[i=1:Num_Cline],Vi[Linedata_candidate[i,1]]-(lL_CL[i,1]*V_min[Linedata_candidate[i,1]]+lL_CL[i,2]*V_min[Linedata_candidate[i,1]]+lL_CL[i,3]*V_min[Linedata_candidate[i,1]]+lL_CL[i,4]*V_min[Linedata_candidate[i,1]]+lL_CL[i,5]*V_max[Linedata_candidate[i,1]]+lL_CL[i,6]*V_max[Linedata_candidate[i,1]]+lL_CL[i,7]*V_max[Linedata_candidate[i,1]]+lL_CL[i,8]*V_max[Linedata_candidate[i,1]])==0)
   @constraint(model,[i=1:Num_Cline],Vi[Linedata_candidate[i,2]]-(lL_CL[i,1]*V_min[Linedata_candidate[i,2]]+lL_CL[i,2]*V_min[Linedata_candidate[i,2]]+lL_CL[i,3]*V_max[Linedata_candidate[i,2]]+lL_CL[i,4]*V_max[Linedata_candidate[i,2]]+lL_CL[i,5]*V_min[Linedata_candidate[i,2]]+lL_CL[i,6]*V_min[Linedata_candidate[i,2]]+lL_CL[i,7]*V_max[Linedata_candidate[i,2]]+lL_CL[i,8]*V_max[Linedata_candidate[i,2]])==0)
...
#--------------------------------------------------------------------------------------------------------------------------------
if G==1
    @objective(model,Min,sum(Linedata_candidate[i,6]*XL[i] for i in 1:Num_Cline)+sum(Gendata_candidate[i,6]*XG[i] for i in 1:Num_Cgen)+sum(Pg[i] for i in 1:Num_gen)+sum(Pgc[i] for i in 1:Num_Cgen)+sum(Qc[i] for i in 1:Num_bus)+sum(Ql[i] for i in 1:Num_bus));
else
    @objective(model,Min,sum(Linedata_candidate[i,6]*XL[i] for i in 1:Num_Cline)+sum(Pg[i] for i in 1:Num_gen)+sum(Qc[i] for i in 1:Num_bus)+sum(Ql[i] for i in 1:Num_bus));
end
return model
...
function get_LB(bounds,RT,K,G)
    model = relax(bounds,RT,K,G);
    ...
    if G==1
        println("\n","XL= ",round.(value.(model[:XL])),"\n")
        println("XG= ",round.(value.(model[:XG])),"\n")
        return BestBound, cost, round.(value.(model[:XL])), round.(value.(model[:XG])), value.(model[:Qc]), value.(model[:Ql])
    else
        println("\n","XL= ",round.(value.(model[:XL])),"\n")
        return BestBound, cost, round.(value.(model[:XL])), value.(model[:Qc]), value.(model[:Ql])
    end
end
