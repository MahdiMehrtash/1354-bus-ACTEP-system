...
Gendata_candidate=CSV.read("Data/Gendata_candidate.csv",DataFrame);
...
RE_max=maximum(V_max)*maximum(V_max)*ones(Num_Eline);
LE_min=-maximum(V_max)*maximum(V_max)*ones(Num_Eline);
LE_max=maximum(V_max)*maximum(V_max)*ones(Num_Eline);
